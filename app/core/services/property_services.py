...
class PropertyServices:
    ...
    async def predict_price(
        ...
        address = await self.find_address_by_zip_code(
        ...
        is_cached = self.check_if_is_cached(
            predict_property=predict_property, address=address
        )
        if is_cached:
            _logger.info(
                f"Cached property - Zip Code: {str(predict_property.zip_code)}"
            )
            return is_cached
    ...
    def cache_property(self, predicted_property: PredictedProperty) -> bool:
        try:
            ...
            redis_conn.conn.setex(
                name=key, value=predicted_property.model_dump_json(), time=300
            )
    ...
    def check_if_is_cached(
        self, predict_property: PredictProperty, address: dict
    ) -> PredictedProperty:
        try:
